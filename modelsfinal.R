load("EDAdrop.RData")
str(EDAdrop)
#Drop occurrenceyear, month, day, dayofyear, dayofweek,and hoodid as it is not needed for training.
EDAdrop2<-subset(EDAdrop, select = -c(2,3,4,5,6,10))
set.seed(234)
ind<-sample(2, nrow(EDAdrop2), replace = TRUE, prob=c(0.7,0.3))
trainset.model<-EDAdrop2[ind == 1,]
testset.model<-EDAdrop2[ind == 2,]
